...
export async function POST() {
    try {
        ...
        for (let i = 0; i < dummyCustomers.length; i++) {
            ...
            // Add 1-5 visit histories per customer
            ...
            for (let v = 0; v < visitCount; v++) {
                ...
                await prisma.visitHistory.create({
                    data: {
                        ...
                        treatment_category: c.desired_treatment,
                        treatment_content: `${c.desired_treatment} ${[30, 45, 60, 90][Math.floor(Math.random() * 4)]}分コース`,
            ...
            results.push({ name: c.name, visits: visitCount });
        }

        return NextResponse.json({ success: true, message: `${results.length}名のダミーデータを投入しました`, results });
    } catch (error) {
